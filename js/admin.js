...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const { data: sessao, error } = await banco.auth.getSession();
    ...
    console.log("Sessão:", sessao.session.user.id);console.log("Perfil:", perfil);console.log("Erro:", erroPerfil);
...
async function gerarQROnline() {
    ...
    const codigo = (typeof data === "object" && data !== null)
        ? (data.codigo || data[0]?.codigo || Object.values(data)[0])
        : data;
...
async function carregarDashboard() {
    ...
    try {
        ...
        if (document.getElementById("qrDisponiveis"))
        ...
        if (document.getElementById("qrAtivados"))
        ...
        if (document.getElementById("ultimoLote")) {
            ...
            document.getElementById("ultimoLote").textContent =
                (ultimoLote && ultimoLote.length > 0)
                    ? (ultimoLote[0].coluna_lote || ultimoLote[0].lote)
                    : "Nenhum";
